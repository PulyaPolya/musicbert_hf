...
if __name__ == "__main__":
    ...
    config =load_config("scripts/finetune_params.yaml")
    ...
    hyperparams_dict = load_baseline_params(config.targets)
    ...
    if config.wandb_project:
            for target in config.targets:
               wandb.log({f"test_{target}_accuracy": results[f"test_{target}_accuracy"]
                        })
            wandb.finish()
